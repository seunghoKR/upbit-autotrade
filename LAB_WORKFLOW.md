...
1. **커밋 메시지 규칙:**
   ...
   - `staging: vX.X.X` : 실험실 배포 (운영자 검증용, `staging` 브랜치)
   - `release: vX.X.0` : 실서버 정식 릴리즈 (`main` 브랜치)

2. **버전 관리 원칙 (Versioning Rules):**
   - **연구실 ➔ 실험실 (Staging):** 기능/디자인 개선 및 버그 수정 시마다 `+0.0.1` 단위로 업데이트 (예: `v3.0.0` ➔ `v3.0.1`)
   - **실험실 ➔ 실서버 (Production):** 검증 완료 후 **가급적 1일 1회만 배포**, `+0.1.0` 단위로 마일스톤 업데이트 (예: `v3.0.1` ➔ `v3.1.0`)
