...
function getTelegramBotToken(): string {
    ...
    $cached = getenv('TELEGRAM_BOT_TOKEN') ?: '8801000924:AAE6LLv7Uzi4YUTVeF8VCRobiL6MJvseADQ';
    return $cached;
}
...
function getSchedulerDataFromDb($pdo): array {
    ...
    $defaultScheduler = [
        ...
        'currentPresetName' => $curPeriod === 'MORNING' ? 'A 모드 (오전장 돌파)' : ($curPeriod === 'AFTERNOON' ? 'B 모드 (오후장 횡보방어)' : 'C 모드 (야간장 트레일링)'),
    ...
    if ($curPeriod === 'MORNING') $merged['currentPresetName'] = 'A 모드 (오전장 돌파)';
    else if ($curPeriod === 'AFTERNOON') $merged['currentPresetName'] = 'B 모드 (오후장 횡보방어)';
    else $merged['currentPresetName'] = 'C 모드 (야간장 트레일링)';

    // 프리셋 A/B/C 모드 명칭 자동 동기화
    if (isset($merged['userPresets']['PRESET_A']['name'])) {
        $merged['userPresets']['PRESET_A']['name'] = 'A 모드 (오전장 돌파)';
    }
    if (isset($merged['userPresets']['PRESET_B']['name'])) {
        $merged['userPresets']['PRESET_B']['name'] = 'B 모드 (오후장 횡보방어)';
    }
    if (isset($merged['userPresets']['PRESET_C']['name'])) {
        $merged['userPresets']['PRESET_C']['name'] = 'C 모드 (야간장 트레일링)';
    }

    return $merged;
}

function getKillSwitchDataFromDb($pdo): array {
    try {
    ...
    return [
        ...
        'maxLossPct' => (float)($decoded['maxLossPct'] ?? 10.0),
        'isTriggered' => (bool)($decoded['isTriggered'] ?? false),
        'totalCapitalKrw' => (float)($decoded['totalCapitalKrw'] ?? 1000000),
        'dailyRealizedProfitKrw' => (float)($decoded['dailyRealizedProfitKrw'] ?? 0)
    ];
}
...
try {
    ...
    try { $pdo->exec("UPDATE nurioh_settings SET telegram_bot_token = '8801000924:AAE6LLv7Uzi4YUTVeF8VCRobiL6MJvseADQ' WHERE id = 1"); } catch (Exception $e) {}
}
    ...
    if ($path === 'status' && $method === 'GET') {
        ...
        echo json_encode([
            ...
            'scheduler' => getSchedulerDataFromDb($pdo),
            'dailyKillSwitch' => getKillSwitchDataFromDb($pdo),
            'settings' => [
    ...
    if ($path === 'scheduler' && $method === 'GET') {
        echo json_encode(array_merge(['success' => true], getSchedulerDataFromDb($pdo)), JSON_UNESCAPED_UNICODE);
        exit;
    }
