...
### 🚀 [릴리즈 25] v4.0.0 그랜드 릴리즈 (A/B/C 모드 체계 일원화 & 실서버/실험실/연구실 동시 배포)
- **작업 일시:** 2026-09-17 22:25 (KST)
- **배경:** 슬롯별 개별 전략 수정 기능과의 중복 혼선을 해소하기 위해 UI/UX 전반에서 시간 종속적인 '오전, 오후, 야간' 단어 및 개념을 완전히 배제하고, 순수 **A 모드(초단타 스캘핑) / B 모드(신고가 돌파) / C 모드(추세 스윙)**로 아키텍처를 일원화하여 v4.0.0 메이저 릴리즈 달성.
- **주요 반영 내역:**
  1. **A/B/C 모드 체계 일원화:**
     - **A 모드 (초단타 스캘핑) ⚡:** `초단타 급등 & VWAP 돌파`
     - **B 모드 (신고가 돌파) 🚀:** `당일 신고가 & 돌파 매수`
     - **C 모드 (추세 스윙) 🌊:** `이평 정배열 & 트레일링 스탑`
     - 스케줄 시간표: `⚡ A 모드 시작`, `🚀 B 모드 시작`, `🌊 C 모드 시작`
  2. **슬롯 카드 내 A/B/C 1초 퀵 스위치 바 제공:** 각 슬롯에서 모드 A/B/C 즉시 전환 복원.
  3. **백엔드/엔진 동기화 (`marketScheduler.js`, `index.php`):** API 응답 및 프리셋 DB 복원 시 신규 A/B/C 모드 명칭 자동 주입.
  4. **🏛️ 3대 환경 전체 동시 배포:**
     - 연구실 (Local Lab `localhost:3000`, `dev` 브랜치)
