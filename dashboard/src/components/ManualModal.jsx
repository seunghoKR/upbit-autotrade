...
export default function ManualModal({ isOpen, onClose, user, onOpenSettings, onOpenMyPage }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-3 sm:p-4 animate-in fade-in overflow-y-auto">
      <div className="bg-slate-900 border-2 border-indigo-500/60 rounded-3xl max-w-3xl w-full p-4 sm:p-6 shadow-2xl shadow-black/90 relative space-y-5 my-auto max-h-[95vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/40 hidden sm:inline">
                  v2.5 정식판
                </span>
              </div>
        ...
        {/* 2. 탭 네비게이션 */}
        <div className="flex items-center gap-2 border-b border-slate-800 pb-2 text-xs font-bold flex-wrap sm:flex-nowrap">
          <button
        ...
        {/* 탭 1: 핵심 자동매매 기능 매뉴얼 */}
        {activeTab === 'MANUAL' && (
          <div className="space-y-4 animate-in fade-in text-xs text-slate-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-2.5 text-[11px]">
                ...
                <div className="bg-slate-900/90 p-3 rounded-xl border border-slate-800 space-y-1">
                  <div className="flex items-center gap-1 font-bold text-purple-300">
                    <span className="w-4 h-4 rounded-full bg-purple-500/20 text-purple-300 flex items-center justify-center text-[10px]">4</span>
            ...
            <div className="bg-slate-950/80 p-4 rounded-2xl border border-slate-800 space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex items-start gap-2.5 p-2.5 bg-slate-900/60 rounded-xl border border-slate-800/80">
                  <span className="px-2 py-0.5 rounded-lg bg-indigo-600 text-white font-bold text-[11px] shrink-0">
                    [ 슬롯 원클릭 선택 ]
                  </span>
                ...
                <div className="flex items-start gap-2.5 p-2.5 bg-slate-900/60 rounded-xl border border-slate-800/80">
                  <span className="px-2 py-0.5 rounded-lg bg-rose-600 text-white font-bold text-[11px] shrink-0">
                    [ 🚨 긴급 강제 매도 ]
                  </span>
                  <p className="text-slate-300 text-[11px]">
                ...
                <div className="flex items-start gap-2.5 p-2.5 bg-slate-900/60 rounded-xl border border-slate-800/80">
                  <span className="px-2 py-0.5 rounded-lg bg-amber-500/20 text-amber-300 border border-amber-500/40 font-bold text-[11px] shrink-0">
                    [ ⚡ 알트 급등 포착 테스트 ]
                  </span>
                  <p className="text-slate-300 text-[11px]">
          ...
          <div className="space-y-4 animate-in fade-in text-xs text-slate-300">
            ...
            <div className="space-y-2.5">
              <div className="bg-slate-950/80 p-3.5 rounded-2xl border border-slate-800 space-y-1.5">
                <h5 className="font-bold text-amber-300 flex items-center gap-1.5 text-xs">
        ...
        {activeTab === 'FEEDBACK' && (
          <div className="space-y-4 animate-in fade-in text-xs">
            {/* 상단 안내 배너 */}
            <div className="bg-gradient-to-r from-amber-500/15 via-orange-500/15 to-indigo-500/15 p-4 rounded-2xl border border-amber-500/30 space-y-1">
            ...
            {/* 의견 작성 폼 */}
            <form onSubmit={handleSubmitFeedback} className="bg-slate-950/80 p-4 rounded-2xl border border-slate-800 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {/* 구분 선택 */}
                <div>
                  <label className="text-slate-400 block mb-1 font-semibold text-[11px]">의견 구분</label>
                  <select
                ...
                {/* 작성자 이름 */}
                <div>
                  <label className="text-slate-400 block mb-1 font-semibold text-[11px]">작성자 / 닉네임</label>
                  <input
                    type="text"
                    value={authorName}
                ...
                {/* 연락처 / 이메일 */}
                <div>
                  <label className="text-slate-400 block mb-1 font-semibold text-[11px]">연락처 / 이메일</label>
                  <input
                    type="text"
                    value={contact}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-slate-100 text-xs focus:outline-none focus:border-amber-500 leading-relaxed placeholder:text-slate-600 resize-none"
                />
              </div>
                ...
                <span className="text-[11px] text-slate-500">
                  {isSubmitted && (
                    <span className="text-emerald-400 font-bold flex items-center gap-1 animate-bounce">
                      <CheckCircle2 className="w-3.5 h-3.5" /> 소중한 의견이 영자에게 성공적으로 전달되었습니다! 💖
                    </span>
                  )}
                </span>
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-black font-extrabold text-xs transition shadow-lg shadow-amber-500/20 flex items-center gap-1.5 cursor-pointer"
                >
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
                {feedbackList.map((item) => (
                  <div key={item.id} className="p-3 bg-slate-950/60 rounded-xl border border-slate-800/80 space-y-1">
                    <div className="flex items-center justify-between text-[11px]">
                      <div className="flex items-center gap-2">
                        <span className="px-2 py-0.2 rounded bg-indigo-500/20 text-indigo-300 font-bold">
                          {item.category}
                        </span>
                        <span className="text-slate-300 font-semibold">{item.author}</span>
                      </div>
                      <div className="flex items-center gap-2 text-slate-500">
                        <span>{item.createdAt}</span>
                        <span className="px-1.5 py-0.2 rounded bg-emerald-500/10 text-emerald-400 font-bold text-[10px]">
                          {item.status}
                        </span>
                      </div>
                    </div>
