        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-bold text-slate-100 flex items-center gap-2">
              ...
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 font-bold hidden sm:inline">
          ...
          return (
            ...
            >
              <div className="space-y-2.5 sm:space-y-3">
                ...
                {isPending ? (
                  /* ⚡ 매수 승인 대기 중 상태 뷰 */
                  <div className="space-y-2 pt-1 bg-amber-950/30 p-3 rounded-xl border border-amber-500/30" onClick={e => e.stopPropagation()}>
                    <div className="text-sm">
                      ...
                      <div className="flex justify-between text-slate-200 text-xs mt-1">
                        <span>주문 예정금:</span>
                        <span className="font-bold text-slate-100 font-mono">
                          {Number(pendingApproval.amount).toLocaleString()}원
                        </span>
                      </div>
                      <p className="text-xs text-amber-200/90 mt-1.5 truncate">
                    ...
                    <div className="flex gap-1.5 pt-1">
                      <button
                        ...
                        className="flex-1 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold text-xs sm:text-sm transition flex items-center justify-center gap-1.5 shadow-md cursor-pointer"
                      >
                  ...
                  /* 설정 수정 폼 */
                  <div className="space-y-2.5 pt-1 border-t border-slate-800 text-xs sm:text-sm" onClick={e => e.stopPropagation()}>
                    <div>
                      <label className="text-xs text-slate-300 block mb-1 font-semibold">1회 매수금액 (원)</label>
                      <input
                        type="number"
                        step="5000"
                        value={editForm.tradeAmountKrw}
                        onChange={(e) => setEditForm({ ...editForm, tradeAmountKrw: Number(e.target.value) })}
                        className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-slate-100 text-sm focus:outline-none focus:border-indigo-500 font-mono font-bold"
                      />
                    </div>
                    <div className="flex items-center justify-between pt-0.5">
                      <label className="text-xs text-slate-300 font-semibold">슬롯 활성화</label>
                      <button
                        type="button"
                      ...
                      >
                        <div className={`w-4 h-4 rounded-full bg-white transition-transform ${
                          editForm.isEnabled ? 'translate-x-4' : 'translate-x-0'
                        }`} />
                      </button>
                    </div>
                    <div className="flex gap-1.5 pt-1.5">
                      <button
                        ...
                        className="flex-1 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs transition"
                      >
                        저장
                      </button>
                      <button
                  ...
                  <div className="space-y-2 pt-0.5">
                    <div className="flex items-center justify-between text-xs sm:text-sm bg-slate-950/40 px-3 py-2 rounded-xl border border-slate-800/60">
                      <span className="text-slate-300 text-xs font-medium">1회 진입금액</span>
                    ...
                    {isHolding ? (
                      ...
                      <div className="space-y-2 pt-0.5">
                        <div className="flex items-end justify-between bg-slate-950/60 p-2.5 rounded-xl border border-slate-800">
                          <div>
                            <span className="text-[11px] text-slate-400 block">실시간 수익률</span>
                            <div className={`text-base font-black flex items-center ${
                              isProfit ? 'text-emerald-400' : 'text-rose-400'
                            }`}>
                              {isProfit ? <ArrowUpRight className="w-4 h-4 mr-0.5" /> : <ArrowDownRight className="w-4 h-4 mr-0.5" />}
                              {isProfit ? '+' : ''}{profitRate.toFixed(2)}%
                            </div>
                          </div>
                          <div className="text-right">
                            <span className="text-[11px] text-slate-400 block">손익금</span>
                      ...
                      /* 대기 상태 배너 */
                      <div className="py-3 px-2.5 text-center rounded-xl bg-slate-950/40 border border-slate-800/50 space-y-1">
                        <div className="flex items-center justify-center gap-1.5 text-xs text-indigo-300 font-bold">
              ...
              {/* 하단: 액션 버튼들 */}
              {!isEditing && !isPending && (
                <div className="flex gap-2 pt-2.5 mt-2.5 border-t border-slate-800/80" onClick={e => e.stopPropagation()}>
                  <button
                    onClick={(e) => handleStartEdit(e, slot)}
                    className="flex-1 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-100 text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer"
                  >
